...
NewActionForm::NewActionForm(QMainWindow *parent)
  ...
  this->cubeCreation = new CylinderCreation(this);
  ...
  connect(ui->pushButton_2, SIGNAL(clicked(bool)), this->prismCreation,
          SLOT(show()));
  connect(this->cubeCreation, SIGNAL(newCubeReady(Cylinder)), this,
          SLOT(newCubeReady(Cylinder)));
...
void NewActionForm::newCubeReady(Cylinder cube) {
