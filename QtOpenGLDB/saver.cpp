...
bool Saver::persist() {
  ...
  this->db.exec(QString("CREATE TABLE " + prismTableName +
                        ...
                        "  zColor NUMERIC(6,4)  "
                        ")"));
  this->saveCubeVertex->prepare(QString("INSERT INTO " + this->cubeTableName +
                                        " VALUES (?, ?, ?, ?, ?, ?)"));
  this->savePrismVertex->prepare(QString("INSERT INTO " + this->prismTableName +
                                         " VALUES (?, ?, ?, ?, ?, ?)"));
...
void Saver::setData(std::vector<Cube> data) { this->data = data; }
